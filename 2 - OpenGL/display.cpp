...
void display() 
{
  ...
  if (numused) {
	...
	for(int i = 0; i < numused; i++) {
		GLfloat lightposnAux[4];
		GLfloat outAux[4];

		lightposnAux[0] = lightposn[i*4];
		lightposnAux[1] = lightposn[1+i*4];
		lightposnAux[2] = lightposn[2+i*4];
		lightposnAux[3] = lightposn[3+i*4];
		
		transformvec(lightposnAux, outAux);	

		lightransf[i*4] = outAux[0];
		lightransf[1+i*4] = outAux[1];
		lightransf[2+i*4] = outAux[2];
		lightransf[3+i*4] = outAux[3];
